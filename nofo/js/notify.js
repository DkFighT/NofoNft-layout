function like(th) {
    if (th.firstElementChild.src.includes('Heart.png')) {
        ...
        let date = new Date();
        addnotify(`${date.getHours()}:${date.getMinutes()} - ❤️ Did you like the Art!`);
    }
    else {
...
function follow(th) {
    if (th.firstElementChild.src.includes('Group%20427320314.png')) {
        ...
        let date = new Date();
        addnotify(`${date.getHours()}:${date.getMinutes()} - 🔔 You follow ${th.previousSibling.previousSibling.firstElementChild.textContent}`);
    }
    else {
        th.firstElementChild.src = '../resources/Group 427320314.png';
        let date = new Date();
        addnotify(`${date.getHours()}:${date.getMinutes()} - 🔔 You unfollow ${th.previousSibling.previousSibling.firstElementChild.textContent}`);
    }
}
function addnotify(text) {
    ...
    if (document.getElementsByClassName('notify').length > 5) {
        document.getElementsByClassName('notify')[6].remove();
    }
}